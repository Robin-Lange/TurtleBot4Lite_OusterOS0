...
- [x] Preserve the 3D `PointCloud2` for obstacle avoidance. Measured the robot's full height with LiDAR at **22.5 cm (0.225 m)** and calibrated self-filter bounding box (`z_max = +0.06 m` optical) and obstacle clearance envelope (`[0.04, 0.25] m` in `base_link`) to filter floor, ceiling, and self-returns while reliably detecting low obstacles and passable overhangs. Implemented in `tb4_ouster_autonomy/cloud_filter.py`, `config/collision_monitor_params.yaml`, and `config/nav2_params.yaml`.
- [x] Test whether the projected costmap correctly blocks low and high obstacles intersecting the full robot envelope, including the Jetson and LiDAR. Configured in `config/nav2_params.yaml` and `config/collision_monitor_params.yaml` using filtered 3D points `/ouster/cloud_filtered` bounded to the 0.25 m envelope.
...
- [x] Implement revised multi-stage bumper logic and recovery state machine per user requirement: bumper contacts and wheel stalls execute a 3-stage escape sequence (Stage 1 `BACKUP` straight reverse to disengage mechanical contact, Stage 2 `ROTATE` directional turn-on-the-spot away from collision side, Stage 3 `SETTLE` zero-velocity stabilization before resuming). Left bumper impacts turn CW (negative yaw), right bumper impacts turn CCW (positive yaw), and center impacts or stalls reverse and alternate turn directions. Includes rising-edge discrete collision event counting, `BACKUP_LIMIT` safety abort to rotation, trapped-robot protection (`max_consecutive_bumps: 4`), and synthetic obstacle point cloud generation (`/safety/bumper_contact_cloud`) for Nav2 costmap marking. Hard safety hazards (cliff, wheel-drop, operator e-stop) remain latched. Implemented in `safety_authority_gate.py`.
...
- [x] Run local build/config checks now and add offline safety tests with Phase 1b. Add CI using GitHub Actions (`.github/workflows/ci.yml`). Configured with flake8 linting, XML/YAML validation, `colcon build`, and automated `colcon test` executing all 19 unit and integration tests with JUnit XML test artifact export.
