...
- [x] Implement directional bumper escape rerouting reflex per user requirement: bumper contacts on the left rotate CW (negative yaw), contacts on the right rotate CCW (positive yaw), and center contacts back up slightly and turn away on the spot, without triggering a latched emergency stop. Hard safety hazards (cliff, wheel-drop, operator e-stop) remain latched. Implemented in `safety_authority_gate.py`.
...
- [x] Run local build/config checks now and add offline safety tests with Phase 1b. Add CI using GitHub Actions (`.github/workflows/ci.yml`). Configured with flake8 linting, XML/YAML validation, `colcon build`, and automated `colcon test` executing all 15 unit and integration tests with JUnit XML test artifact export.
